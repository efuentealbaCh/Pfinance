...
export default function DashboardAnalytics({ accounts }: DashboardAnalyticsProps) {
    ...
    const summary = data?.summary || { totalIncome: 0, totalExpense: 0, balance: 0, currency: 'CLP' };
        ...
        if (budgetProgress.length > 0 && !alertsShown) {
            ...
            alertBudgets.forEach((b: any) => {
                notifications.show({
                    ...
                    message: `Has gastado ${formatMoney(b.spent, currency)} de ${formatMoney(b.amount, currency)} (${b.percentage}%)`,
                        ...
                        <div style={{ height: 300, width: '100%' }}>
                            <ResponsiveContainer>
                                <AreaChart data={cumulativeChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    />
                                    <YAxis
                                        tickFormatter={(val) => formatMoney(val, currency, { compact: true })}
                                        stroke="#5C5F66"
                                        fontSize={12}
                                    />
